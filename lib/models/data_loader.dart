...
class DataLoader {
  ...
  void addSchoolScore(SchoolScore score) {
    if (allScores.containsKey(score.uid)) {
      print(
          "UID ${score.uid}} is shared by '${score.schoolName}' and '${allScores[score.uid]!.schoolName}'. The former is not added to allScores.");
    } else {
  ...
  void requestSchoolData(int uid) {
    ...
    if (!allSchools.containsKey(uid)) {
      print(
          "Data for UID ${uid} does not exist, an empty default is used instead.");
