...
BEGIN
    ...
    UNION ALL
	SELECT "AverageHateCrime" as "Name", sum(YEARLYHATECRIME)/sum(ENROLLTOT) FROM merged_ipeds_css_irs.GenderFair2024
    UNION ALL
	SELECT "AverageVAWA" as "Name", sum(YEARLYVAWA)/sum(ENROLLTOT) FROM merged_ipeds_css_irs.GenderFair2024;
END$$
