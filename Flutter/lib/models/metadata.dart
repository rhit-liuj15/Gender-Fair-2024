...
class Metadata {
  ...
  static final Map<int, MetadataCategory> groupMapping = <int, MetadataCategory>{};
  ...
  Metadata._privateConstructor();
	...
	void addEntry(int group, String abbreviation, String description) {
		try {
			switch (group) {
				case 0:
					getMetadataCategory(int.parse(abbreviation)).setGroupDescription(description);
				default:
  ...
  MetadataCategory({
	...
	void addEntry(String abbreviation, String description) {
		if (metadataPairs.containsKey(abbreviation)) {
			throw Exception("Duplicate entry on abbreviation '$abbreviation' in group $categoryNumber ($categoryDescription).\nExisting: ${metadataPairs[abbreviation]}\nNew: $description");
		}
